...
class DeepTrace_DataGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self, sample_ids, observation_key_triplets, bugDir, codeDir, imgDir, jsonConfigs, useImgFlag=False, shuffle=True):
        ...
        self.bug_embedding = body["bug_embedding"]
        self.code_dim1_max_word_features = body["code_dim1_max_word_features"]
        self.code_dim2_max_word_features = body["code_dim2_max_word_features"]
    ...
    def load_method_img(self, method_id):
        ...
        file_path = "/media/iwona/Optane/Project_BugLoc/compute_canada/input/images/" + file_name
        #file_name = method_id + ".npy"
        #img_mat = np.load(self.imgDir + "vectorized/" + file_name)
        #img_mat = np.load(file_path, allow_pickle=True)
        #img_mat = img_mat.astype('float64')
        #img_mat = keras_img.load_img(file_path, target_size=(128, 512))
        #img_mat = keras_img.img_to_array(img_mat)
    ...
    def load_bug(self, bug_id):
        ...
        file_path = os.path.join(self.bugDir, "vectorized", file_name)
        #bug_embedded = np.load(self.bugDir + "vectorized/" + file_name)
        bug_embedded = np.load(file_path)
    ...
    def load_method(self, method_id):
        ...
        file_path = os.path.join(self.codeDir, "vectorized", file_name)
        #code_matrix = np.load(self.codeDir + "vectorized/" + file_name)
        code_matrix = np.load(file_path)
...
if __name__ == "__main__":
    ...
    exp_folder = "run00"
    config_file = results_dir + "meta_hyperparams.json"
